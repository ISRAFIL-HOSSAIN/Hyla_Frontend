...
const CreateTransport = () => {
  ...
  const initialValues = {
    ...fieldNames.reduce((acc, fieldName, index) => {
      // // const defaultValue = fieldTypes[index] === "number" ? 0 : "";
  ...
  const handleCreate = async (formValues: any) => {
    ...
    try {
      const apiResponse = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}trackable-transports/create`,
        {
          method: "POST",
          headers: {
      ...
      if (apiResponse.ok) {
        ...
        toast.error(
          "Transport create succeeded, but failed to store details in the database."
        );
      }
    } catch (error) {
          ...
          >
            <Box mt={3}>
              <form onSubmit={formik.handleSubmit}>
                <Card>
                  <CardHeader title="Create Transports" />
                  <Divider />
                  <CardContent>
...
CreateTransport.getLayout = (page: any) => (
  <AuthGuard>
