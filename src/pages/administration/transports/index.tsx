...
import { TransportListTable } from "src/components/Tables/transportTabel";
  ...
  data.filter((item: any) => {
    if (filters.query) {
      ...
      const properties = ["transportName"];

      properties.forEach((property) => {
        // Check if data[property] is defined before calling toLowerCase()
        if (
          data[property] &&
  ...
  const applyPagination = (
    ...
    page: number,
    rowsPerPage: number
  ): any => data.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
  
  const TransportList: NextPage = () => {
    ...
    const getData = useCallback(async () => {
      try {
        ...
        const apiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}trackable-transports/getAll`;
  
        const queryParams = new URLSearchParams();
        queryParams.append("transportName", filters.query);
          ...
          <Container maxWidth="xl">
            <Box sx={{ mb: 4 }}>
              <Grid container justifyContent="space-between" spacing={3}>
                <Grid item>
                  <Typography variant="h4">Transports Details</Typography>
                </Grid>
                <Grid item>
                  <NextLink
                    href="/administration/transports/create"
                    passHref
                  >
                    <Button
  ...
  TransportList.getLayout = function (page) {
    return (
      <AuthGuard>
